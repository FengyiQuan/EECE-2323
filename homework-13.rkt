...
(define-type VAL
  [BogusV]
  [RktV  Any]
  [FunV (Listof Symbol) (ENV -> VAL) Boolean]
  ; (Listof Symbol) (Listof TOY) ENV Boolean] ; `byref?' flag
...
(define (extend-rec names compiled-exprs env)
  (define new-env
    (extend names (map (lambda (_) the-bogus-value)
                       compiled-exprs)
            env))
  ;; note: no need to check the lengths here, since this is only
  ;; called for `bindrec', and the syntax make it impossible to have
...
;; complie a list of expressions, returns the last value.
...
(define (compile-body exprs)
  ...
  ;; return a single compiled-expression function that runs them one
  ;; by one and returns the last value
  (define (compile-body* compiled-exprs env)
    (let ([1st ((first compiled-exprs) env)]
          [rest (rest compiled-exprs)])
      (if (null? rest)
          1st
          (compile-body* rest env))))
  (let ([compiled-exprs (map compile exprs)])
    (lambda ([env : ENV]) (compile-body* compiled-exprs env))))
...
(define (compile-get-boxes exprs)
  ...
  (let ([getters (map compile-getter exprs)])
    (lambda (env)
      (map (lambda ([getter : (ENV -> (Boxof VAL))]) (getter env))
           getters))))
...
(define (compile expr)
  ...
  (cases expr
    ...
    [(Set name new)
     (lambda ([env : ENV])
       (set-box! (lookup name env) ((caller env) (compile new)))
       the-bogus-value)]
    ...
    [(Fun names bound-body)
     (lambda ([env : ENV]) (FunV names bound-body env #f))]
    [(RFun names bound-body)
     (lambda ([env : ENV]) (FunV names bound-body env #t))] 
    [(Call fun-expr arg-exprs)
     (lambda ([env : ENV])
       (let ([fval ((caller env) (compile fun-expr))]
             [arg-vals (lambda () (map (caller env) (map compile arg-exprs)))])
         (cases fval
           [(PrimV proc) (proc (arg-vals))]
           [(FunV names compiled-body byref?)
              (if byref?
                  (compiled-body (raw-extend names
                                             ((compile-get-boxes arg-exprs) env)
                                             compiled-body))
                  (compiled-body (extend names (arg-vals) compiled-body)))]
    ...
    [(If cond-expr then-expr else-expr)
     (let ([compiled-cond (compile cond-expr)]
           [compiled-then (compile cond-expr)]
           [compiled-else (compile else-expr)])
       (lambda ([env : ENV]) 
      
         (if (cases ((caller env) compiled-cond)
               [(RktV v) v] ; Racket value => use as boolean
               [else #t])   ; other values are always true
             ((caller env) compiled-then)
             ((caller env) compiled-else))))]))
...
;;; ==================================================================
(test (run "{bindrec {{f1 {fun {x} {if {= x 0} 0 {f2 {- x 1}}}}}
                      {f2 {fun {x} {if {= x 0} 0 {f1 {- x 2}}}}}}
              {f1 27}}")
      => 0)
(test (run "{bindrec {{iseven? {fun {x} {if {= x 0}
                                        0
                                        {isodd? {- x 1}}}}}
                       {isodd? {fun {x} {if {= x 1}
                                       1
